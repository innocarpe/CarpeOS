...
export function evaluateAgenticGate(input: {
  ...
  // Hold-first: auto-promote only when explicitly enabled and allowlisted.
  if (input.allow_auto_promote === true && kind_allowlisted && input.cite_ok && input.secret_ok) {
    return {
      schema: "carpeos.agentic.gate-result/v1",
      policy_version: AGENTIC_POLICY_VERSION,
      decision: "promote",
      reason_codes: ["allowlist_auto_promote", "cite_ok"],
      features,
    };
  }
