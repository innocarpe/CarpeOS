...
import { buildP02SandboxReceipt, buildSandboxProbeObservation } from "../product4/p02-runner.mjs";
import {
  digestJson,
...
test("M4 refuses missing and forged sandbox evidence before candidate execution", () => {
  ...
  const cli = mkdtempSync(join(tmpdir(), "product4-cli-sandbox-"));
  const home = mkdtempSync(join(tmpdir(), "product4-home-sandbox-"));
  const output = mkdtempSync(join(tmpdir(), "product4-output-sandbox-"));
  const observedProbe = buildSandboxProbeObservation({
    identity: {
      head_sha: headSha,
      base_sha: "c".repeat(40),
      tree_sha256: identity.tree_sha256,
